...
|---|---|---|---|---|
| **30 fps, 노이즈 ≤10, 60 BPM 이상** (일반적인 촬영 조건) | reference | **97%** | **100%** | 0.13% |
| 30 fps, 노이즈 ≤10, 30 BPM 포함 | reference | 91% | 93% | 0.14% |
| 30 fps, 노이즈 ≤10 | consecutive | 84% | 86% | 0.13% |
| 30 fps, 노이즈 ≤10, 텍스처 강함 | piv | 80% | 89% | 0.23% |
| 30 fps, 노이즈 ≤10, 텍스처 약함 | piv | 78% | 84% | 0.27% |
| 15 fps 이상, 노이즈 ≤10, 60 BPM 이상 | reference | 80% | 86% | 0.16% |
| 전체 그리드 (극단 조건 포함) | reference / consecutive / piv | 51% / 65% / 52% | 57% / 74% / 64% | 0.3% / 0.4% / 0.8% |

읽는 법과 실제로 확인된 약점:
- **틀릴 때는 거의 항상 박동을 놓치는 쪽**(과소검출)이지 없는 박동을 만들어내진 않았습니다(전
  그리드에서 과다검출 0건). 그리고 박동 수가 맞은 경우 BPM 오차는 1% 미만입니다.
- **노이즈가 심하면(표준편차 20) reference 모드가 무너집니다** — 대부분 박동 1개로 검출(정확 일치
  3%). 기준 프레임과의 차이가 노이즈 바닥에 묻혀 피크 민감도(`prominence_frac`)가 못 잡는 것입니다.
  화면이 거칠게 지글거리는 영상이면 `consecutive` 모드(같은 조건에서 40-67%)를 쓰거나
  `prominence_frac`을 낮추세요. 이 조건의 자동 처리(프레임 사전 블러 등)는 향후 개선 항목입니다.
- **10 fps는 부족합니다** — reference 34%, consecutive 52%. 박동당 프레임이 몇 장 안 되면 피크가
  뭉개집니다. 15 fps 이상, 가능하면 30 fps로 찍으세요.
- **PIV 모드는 30 fps + 텍스처가 있을 때만 reference에 근접**하고, 프레임속도가 낮거나 노이즈가
  크면 더 빨리 나빠집니다(10 fps에서 38%, 노이즈 20에서 21%). 벡터장이 필요할 때만 쓰고 박동 수
  자체는 reference 모드를 기준으로 삼는 게 안전합니다.
- 30 BPM(느린 박동)에서 특정 박동 시퀀스가 노이즈와 무관하게 5개 중 3개로 잡히는 경우가 있었습니다
  (같은 seed에서 재현). 자동 튜닝된 스무딩/최소 간격이 느린 박동에서 아직 최적이 아닐 수 있다는
  뜻이라, 느린 박동 영상은 결과 플롯을 눈으로 확인하세요.
